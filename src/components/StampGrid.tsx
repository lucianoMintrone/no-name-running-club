...
export function StampGrid({ totalStamps = 30, onAddDay }: StampGridProps) {
  ...
  return (
    <div className="grid grid-cols-5 gap-4">
      {stamps.map((number) => (
        <div
          key={number}
          className="flex h-14 w-14 items-center justify-center rounded-full border-2 border-zinc-300 bg-white text-base font-semibold text-zinc-400 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-500"
        >
          {number}
        </div>
      ))}
